...
import { ArrowLeft, Users, Globe, Lock, Zap } from 'lucide-react';

export default function ReactionBattleSetupPage() {
  ...
  return (
    <div className="min-h-screen game-bg p-6 flex flex-col items-center">
      <div className="w-full max-w-2xl mb-8 flex items-center justify-between">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-red-500 tracking-tight text-center flex items-center gap-2">
          <Zap className="text-yellow-400" /> REACTION BATTLE
        </h1>
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        {/* Local Mode */}
        <button
          onClick={() => navigate('/reaction-battle/play')}
          className="w-full group bg-surface-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 text-left hover:border-yellow-500/50 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-yellow-500/50 opacity-0 group-hover:opacity-100 transition-opacity" />
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            Put the device between you. The screen is split in half. First to tap the target wins the round! First to 5 points wins.
          </p>
        </button>
        ...
        <div className="w-full bg-surface-900/30 backdrop-blur-xl border border-white/5 rounded-3xl p-6 relative overflow-hidden opacity-75">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Test reflexes globally</p>
            </div>
          </div>
        </div>
