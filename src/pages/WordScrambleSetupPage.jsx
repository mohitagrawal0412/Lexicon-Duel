...
import { ArrowLeft, Users, Globe, Lock, Type } from 'lucide-react';

export default function WordScrambleSetupPage() {
  ...
  return (
    <div className="min-h-screen game-bg p-6 flex flex-col items-center">
      <div className="w-full max-w-2xl mb-8 flex items-center justify-between">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500 tracking-tight text-center flex items-center gap-2">
          <Type className="text-orange-400" /> WORD SCRAMBLE
        </h1>
      ...
      <div className="w-full max-w-md space-y-6">
            ...
            {['EASY', 'MEDIUM', 'HARD'].map(level => (
              ...
              >
                ...
                <div className="text-xs opacity-75 font-normal">
                  {level === 'EASY' ? '5 letters' : level === 'MEDIUM' ? '7 letters' : '9+ letters'}
                </div>
              </button>
            ))}
          </div>
        ...
        {/* Local Mode */}
        <button
          onClick={() => navigate('/word-scramble/play', { state: { difficulty } })}
          className="w-full group bg-surface-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 text-left hover:border-orange-500/50 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden"
        >
          <div className="absolute top-0 left-0 w-full h-1 bg-orange-500/50 opacity-0 group-hover:opacity-100 transition-opacity" />
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            Tap the scrambled letter tiles to form the target word. First player to form the word correctly wins the round!
          </p>
        </button>
        ...
        <div className="w-full bg-surface-900/30 backdrop-blur-xl border border-white/5 rounded-3xl p-6 relative overflow-hidden opacity-75">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Unscramble words globally</p>
            </div>
          </div>
        </div>
