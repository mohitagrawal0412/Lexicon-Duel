...
import { ArrowLeft, Users, Globe, Lock, Image as ImageIcon } from 'lucide-react';

export default function MemoryMatchSetupPage() {
  ...
  return (
    <div className="min-h-screen game-bg p-6 flex flex-col items-center">
      <div className="w-full max-w-2xl mb-8 flex items-center justify-between">
        ...
        <h1 className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-500 tracking-tight text-center flex items-center gap-2">
          <ImageIcon className="text-purple-400" /> MEMORY MATCH
        </h1>
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="bg-surface-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6">
          <h3 className="text-white font-bold mb-4">Select Grid Size</h3>
            ...
            {['EASY', 'HARD'].map(level => (
              ...
              >
                ...
                <div className="text-xs opacity-75 font-normal">
                  {level === 'EASY' ? '4x3 (6 Pairs)' : '4x4 (8 Pairs)'}
                </div>
              </button>
            ))}
          </div>
        ...
        {/* Local Mode */}
        <button
          onClick={() => navigate('/memory-match/play', { state: { difficulty } })}
          className="w-full group bg-surface-900/60 backdrop-blur-xl border border-white/10 rounded-3xl p-6 text-left hover:border-purple-500/50 transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden"
        >
          ...
          <p className="text-sm text-gray-500 leading-relaxed">
            Memorize the board, then race to match the pairs! The first player to clear their board wins.
          </p>
        </button>
        ...
        <div className="w-full bg-surface-900/30 backdrop-blur-xl border border-white/5 rounded-3xl p-6 relative overflow-hidden opacity-75">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Compete with memories globally</p>
            </div>
          </div>
        </div>
