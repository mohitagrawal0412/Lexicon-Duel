...
const Grid = ({ size, onBack }) => {
    ...
    const handleInputChange = (index, value) => {
        ...
        updatedGrid[index] = value.toUpperCase().charAt(0);  // Update the character
    ...
    const handleMouseDown = (index) => {
        ...
        setIsSelecting(true);
        setSelectionDirection(null); // Reset direction
        setError(null);
    };
    ...
    const handleMouseEnter = (index) => {
        if (isSelecting) {
            ...
            if (selectionDirection === "vertical" && colStart === colCurrent) {
    ...
    return (
        <div className="flex min-h-screen bg-gray-100">
            {/* Left Section for Player Info, Controls, and Messages */}
            <div className="flex flex-col items-center p-4 space-y-4 w-1/4 bg-gray-200">
                ...
                {/* Error and Selected Word Messages */}
                {error && (
                ...
                >
                    {gridData.map((cell, index) => (
                        <input
                            ...
                            className={`flex items-center justify-center border border-gray-300 bg-white h-10 text-gray-700 font-medium text-center uppercase focus:outline-none focus:ring-2 ${selectedIndices.includes(index)
                                ? 'bg-blue-200'
                                : ''
                                }`}
                        />
